...
if($Pack -eq "true") {
    ...
    $DeploymentAbsolutePath = Resolve-Path $DeploymentPath  | select -ExpandProperty Path 
    $command = $PluginBuilderAbsolutePath + " --path=" + $pluginLibrary + " --output=" + $DeploymentAbsolutePath + " --manifest=true --pack=true"
    Write-Host $command
    iex $command;
}